...
| Method | Route | Description |
|---|---|---|
| GET | `/api/network/students` | Search students. `?search=`, `?major=`, `?courseId=`. Returns public profile fields only. |
| GET | `/api/network/connections` | Accepted connections. |
| GET | `/api/network/requests` | Incoming and outgoing pending requests. |
| POST | `/api/network/requests` | Body: receiverId. 409 if one already exists. |
| PUT | `/api/network/requests/{id}` | Body: status (Accepted / Declined). Only the receiver may accept. |
| DELETE | `/api/network/connections/{id}` | Remove a connection. |
| POST | `/api/network/block` | Body: userId. Blocks messages and requests. |

### Messages — `/api/messages`

| Method | Route | Description |
|---|---|---|
| GET | `/api/messages/conversations` | Conversation list with last message and unread count. |
| GET | `/api/messages/conversations/{userId}` | Message history with one user. Paginated. |
| POST | `/api/messages` | Body: receiverId, content. 403 if not connected or blocked. |
| PUT | `/api/messages/{id}/read` | Mark read. |

---

## External contacts — `/api/contacts` *(Later)*
