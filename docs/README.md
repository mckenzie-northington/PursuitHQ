...
1. ~~**AI provider**~~ — **Decided: Google Gemini** (`gemini-3.8-flash`), behind an `IAiService` interface. **Billing is enabled**, which both lifted the free tier's 20-requests-per-minute ceiling and changed the data terms; cost is roughly a penny per request. See `Architecture.md` §5d and `START-HERE.md` §5a. ~~**Open sub-item:** check whether the paid tier still trains on submitted content~~ — **answered, 22 September 2026:** it does not. Google's Gemini API Additional Terms state that on paid services it does not use prompts or responses to improve its products; it does still log them briefly for abuse detection and legal compliance. The privacy policy says both.
...
9. **File storage has two implementations behind one interface.** `LocalFileStorageService` for development, `S3FileStorageService` (Cloudflare R2) for production, chosen by `FileStorage:Provider`. It falls back to local disk when an S3 credential is missing rather than refusing to start, because a half-configured bucket should degrade to something that works while you fix it — the startup log names which was chosen, since silently writing to the wrong place is the bad outcome. Stored keys match between the two, so switching moves bytes rather than rows.
